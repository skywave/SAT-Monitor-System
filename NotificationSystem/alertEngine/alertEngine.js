...
class alertEngine {
  ...
  async _evaluateTrunkStatus(trunk) {
    ...
    //   Standard:  trunk.status / trunk.trunk_status  (text like "registered")
    //   Yeastar:   trunk.enb_call_status              (number: 1=enabled, 0=disabled)
    ...
    if (trunk.status !== undefined) {
      // Standard text status field
      status = String(trunk.status).toLowerCase();
    } else if (trunk.trunk_status !== undefined) {
      status = String(trunk.trunk_status).toLowerCase();
    } else if (trunk.enb_call_status !== undefined) {
      // Yeastar numeric field — convert to text
      status = trunk.enb_call_status === 1 ? 'registered' : 'unregistered';
    } else {
      status = 'unknown';
    }
...
//   // ── 3. CDR Metric Thresholds ─────────────────────────────────────────────
//   async _pollMetrics() {
//     var metricsMap;
//     try {
//       var windowMinutes = Math.ceil(config.polling.intervalMs / 60000) + 1;
